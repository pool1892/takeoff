...
CONTEXT_LINK = "[Project context](https://github.com/pool1892/takeoff/blob/codex/supplier-epic/docs/demo-context.md)"
...
class AmbiguousRecords:
    ...
    def publish_catalog(self, market, run_id, vendor_id):
        ...
        title = f"{catalog['vendor']['name']} — Product catalog"
        rows = [[p.get(key) for key in ("id", "name", "unit", "pack_size", "list_price", "stock",
                                      "minimum_quantity", "substitution_for", "source")]
                for p in catalog["products"]]
        content = (f"# {title}\n\n"
                   "A publication-time catalog snapshot. Request a current quote to confirm stock and delivery capacity.\n\n"
                   f"Catalog reference: {run['scenario_version']} / {run_id}\n\n"
    ...
    def publish_offer(self, offer):
        ...
        title = f"Quote {quote_id} — Revision {public['revision']} — {status.title()}"
        content = (f"# {title}\n\n"
    ...
    def publish_catalog_sheet(self, market, run_id, vendor_id):
        ...
        payload = {"title": f"{catalog['vendor']['name']} — Catalog snapshot",
